...
const API_TRUE_VALUES = new Set<unknown>([
	1,
	21,
	23,
	'1',
...
export class HarviaFenix extends utils.Adapter {
	...
	private async fetchConfig(): Promise<boolean> {
		try {
			...
			this.log.info(
				`API configuration loaded: Data=${this.dataBaseUrl}, Device=${this.deviceBaseUrl}, Events=${this.eventsBaseUrl}, Partner=${this.partnerId}`,
			);
			return true;
		} catch (err) {
		...
		try {
			...
			if (devices.length > 0) {
				...
				if (devices.length > 1 && !this.config.deviceId) {
					this.log.warn(
						`Multiple devices (${devices.length}) found in your Harvia account, but no Device ID is specified in the adapter configuration! The adapter will default to the first discovered device. Please configure a specific Device ID for each instance to avoid control conflicts.`,
					);
				}
	 ...
	 */
	public async processEventsResponse(data: unknown): Promise<void> {
	 ...
	 */
	public static parseEvents(data: unknown): HarviaEventItem[] {
		if (!data) {
			return [];
		}
		if (Array.isArray(data)) {
			return data as HarviaEventItem[];
		}
		if (typeof data === 'object') {
			const obj = data as Record<string, unknown>;
			if (Array.isArray(obj.events)) {
				return obj.events as HarviaEventItem[];
			}
			if (Array.isArray(obj.items)) {
				return obj.items as HarviaEventItem[];
			}
			if (Array.isArray(obj.data)) {
				return obj.data as HarviaEventItem[];
			}
			if (obj.data && typeof obj.data === 'object') {
				const nested = obj.data as Record<string, unknown>;
				if (Array.isArray(nested.events)) {
					return nested.events as HarviaEventItem[];
				}
				if (Array.isArray(nested.items)) {
					return nested.items as HarviaEventItem[];
				}
			}
	 ...
	 */
	public static getEventTimestamp(e: HarviaEventItem): number {
	 ...
	 * @param e - The Harvia event item.
	 */
	public static determineEventSeverity(e: HarviaEventItem): 'info' | 'warn' | 'error' | 'critical' {
	 ...
	 */
	public static evaluateSafetyStatus(events: HarviaEventItem[]): { tripped: boolean; reason: string } {
		...
		switch (stateId) {
			...
			case 'targetTemp': {
				...
				if (Number.isNaN(val) || val < minTemp || val > maxTemp) {
					this.log.error(
						`Invalid target temperature (${state.val}°C) received. Range: ${minTemp}-${maxTemp}°C.`,
					);
			...
			case 'maxDuration': {
				const configuredMaxDuration =
					typeof this.config.maxDuration === 'number' ? this.config.maxDuration : MAX_MAX_DURATION;
				const val = typeof state.val === 'number' ? state.val : Number.parseFloat(String(state.val));
				if (Number.isNaN(val) || val < MIN_MAX_DURATION || val > configuredMaxDuration) {
					this.log.error(
						`Invalid max duration (${state.val} min) received. Range: ${MIN_MAX_DURATION}-${configuredMaxDuration} min.`,
					);
