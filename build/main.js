...
class HarviaFenix extends utils.Adapter {
     ...
     */
    calculateHeatingPrognosis = async (heatOn, currentTemp, targetTemp) => {
        const now = Date.now();
        ...
        if (activeHeatingMinutes >= 10 && windowMinutes >= 3) {
            if (liveRate < 0.5 * historicalRate) {
                ...
                if (!anomalyState?.val) {
                    ...
                    this.log.info(`⚠️ Heating rate (${liveRate.toFixed(2)} °C/min) is significantly below historical average (${historicalRate.toFixed(2)} °C/min). Check sauna door or heater element.`);
                }
            }
            else {
                const anomalyState = await this.getStateAsync('info.heatingAnomaly');
